...
export default function LoginPage() {
  ...
  const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if ("error" in result && result.error) {
      setErrorMessage("Neplatné přihlašovací údaje. Zkuste to prosím znovu.");
      return;
    }
        ...
        <div className="rounded-3xl border border-border/60 bg-card p-8 shadow-xl flex flex-col gap-8">
          ...
          <div className="space-y-1.5 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Zadejte své údaje pro přístup do pracovního prostoru.
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Link href="/recovery" className="text-[10px] font-bold text-blue-600 dark:text-blue-400 hover:underline uppercase tracking-widest">
                  Zapomenuté heslo?
                </Link>
              </div>
              <div className="relative">
            ...
            {errorMessage && (
              <div className="mb-4 rounded-md border border-red-200 bg-red-50 p-3 text-center text-sm text-red-500">
                {errorMessage}
              </div>
            )}
