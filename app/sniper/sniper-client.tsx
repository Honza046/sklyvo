...
export function SniperClient() {
  ...
  const [pitchType, setPitchType] = useState<
    (typeof PITCH_OPTIONS)[number]["value"]
  >(PITCH_OPTIONS[0].value);
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (fullText.includes("===DELIC===")) {
        ...
        setSubjects(
          rawSubjects.length > 0 ? rawSubjects : ["Dotaz k Vašemu webu"],
        );
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-10 pb-20">
      {/* HLAVIČKA */}
      <div>
        <h2 className="sk-type-h1">Direct Outreach Engine</h2>
        <p className="mt-2 text-base text-muted-foreground max-w-2xl">
          Generování e-mailu na míru pro konkrétní cíl na základě analýzy webu a
      ...
      <div className="flex flex-col gap-8 rounded-xl border border-border/60 bg-card p-6 shadow-sm">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          >
            {loading ? (
              <>
                <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Analyzuji...
              </>
            ) : (
              "Vygenerovat email"
            )}
          ...
          <Button
            variant="outline"
            className="flex items-center justify-center p-0 h-11 w-11 border-border/80"
          >
      ...
      {loading && (
        <div className="rounded-xl border border-border/60 bg-muted/30 p-6 animate-pulse">
          <p className="text-sm text-muted-foreground font-mono whitespace-pre-wrap">
            {rawStream || "Inicializuji Sklyvo Sniper Engine..."}
          </p>
        </div>
      )}
      ...
      {isDone && (
        <div className="flex flex-col gap-8 rounded-xl border border-border/60 bg-card p-6 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <h3 className="sk-type-h2">Vygenerovaný e-mail</h3>
            <div className="flex flex-wrap gap-2">
              <span className="flex items-center bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold tracking-wide border border-blue-100">
                🎯 B2B SaaS
              </span>
              <span className="flex items-center bg-gray-50 text-gray-700 px-3 py-1 rounded-full text-xs font-semibold tracking-wide border border-gray-200">
                🤝 Přátelský
              </span>
              <span className="flex items-center bg-gray-50 text-gray-700 px-3 py-1 rounded-full text-xs font-semibold tracking-wide border border-gray-200">
                🌍 CS
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  variant="outline"
                  className="flex h-11 w-11 shrink-0 items-center justify-center p-0 text-muted-foreground hover:text-foreground"
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button
              ...
              className="h-11 px-5 font-semibold text-foreground"
            >
