...
export type VerifiedSession = {
  userId: string;
  /** Bumped on password change / revoke-all to invalidate stolen cookies. */
...
/** Full claims from a signed session cookie, or null if invalid. */
export async function verifySessionClaims(
  ...
  try {
    ...
    ) as { sub?: unknown; exp?: unknown; sv?: unknown };
    if (
    ...
    const sessionVersion =
      typeof claims.sv === "number" && Number.isFinite(claims.sv)
        ? Math.max(0, Math.floor(claims.sv))
        : 0;
