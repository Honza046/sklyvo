...
async function resolveOutboundConnection(
 ...
 // Cross-tenant lock: osobní schránka jen pokud user patří do workspace.
 if (!(await assertUserInWorkspace(uid, workspaceId))) {
 return null;
 }
 const userConnection = await loadUserConnection(uid);
 if (isConnected(userConnection)) {
 return { connection: userConnection, owner: { kind: "user", userId: uid } };
 }
 }
...
async function createOutboundTransporter(
 ...
 const secret = connection.smtpSecret ? decryptSecret(connection.smtpSecret) : null;
 ...
 clientId,
 clientSecret,
 refreshToken: decryptSecret(connection.googleRefreshToken) ?? undefined,
 accessToken,
 },
 });
