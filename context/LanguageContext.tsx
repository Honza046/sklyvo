...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    return subscribeLanguageChange(({ appLanguage }) => {
      setLanguageState((current) => {
        if (current === appLanguage) return current;
        document.documentElement.lang = HTML_LANG[appLanguage];
        return appLanguage;
      });
    });
  }, []);
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      // Re-read after await so a login/landing toggle during boot is not overwritten.
      const storedLang = readAppLanguage();
      if (storedLang && enabled.has(displayCodeForLanguage(storedLang))) {
        writeLanguage(storedLang);
        return;
      }

      // Keep an explicit choice even if the regional chip set is still settling.
      if (storedLang) {
        writeLanguage(storedLang);
        return;
      }
