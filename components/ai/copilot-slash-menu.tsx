...
export function CopilotSlashMenu({
  ...
  useEffect(() => {
    const active = listRef.current?.querySelector<HTMLElement>(`[data-slash-index="${activeIndex}"]`);
  ...
  return (
    ...
    >
      {filtered.map((command, index) => (
        ...
        >
          <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">{command.label}</span>
...
export function filterSlashCommands(commands: SlashCommand[], query: string): SlashCommand[] {
  return commands.filter((command) => command.label.toLowerCase().startsWith(query.toLowerCase()));
}
