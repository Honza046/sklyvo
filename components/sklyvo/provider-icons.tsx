import { LINKEDIN_ARTWORK } from "@/components/sklyvo/linkedin-artwork";
...
/**
 * White circular LinkedIn badge — the supplied artwork, with the "in" knocked
 * out of the disc so the button colour shows through it.
 */
export function LinkedInIcon({ size = 20 }: { size?: number }) {
  return (
    // an inlined data URI: nothing for next/image to fetch or optimise
    // eslint-disable-next-line @next/next/no-img-element
    <img
      src={LINKEDIN_ARTWORK}
      alt=""
      width={size}
      height={size}
...
export function AppleIcon({ size = 17 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden>
      {/* takes its colour from the button, so it works on light and dark */}
      <path
        fill="currentColor"
